  ...
  const TableColumn* TableDesc::GetColumn(int pos) {
	...
	void TableDesc::SetPrimaryKey(vector<string> priCols)
	{
		...
		for (string col : priCols) {
			...
			TableColumn* tc = _vctColumn[iter->second];
			vct.push_back(tc);
		...
		_mapIndexColumn.insert(pair<string, vector<TableColumn*>>(PRIMARY_KEY, vct));
	...
	void TableDesc::AddSecondaryKey(string indexName, IndexType indexType, vector<string> colNames)
	{
		...
		_mapIndexColumn.insert(pair<string, vector<TableColumn*>>(indexName, vct));
