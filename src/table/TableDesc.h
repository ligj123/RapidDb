...
public:
  ...
  const vector<PersistColumn*>& GetPrimaryKeyColumns() const { return _mapIndexColumn.at(PRIMARY_KEY); }
  const unordered_map<string, vector<PersistColumn*>>& GetMapIndexColumns() const { return _mapIndexColumn; }
