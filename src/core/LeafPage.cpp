...
namespace storage {
  ...
  int32_t LeafPage::SearchRecord(const LeafRecord& rr, bool& bFind, bool bInc, int32_t start, int32_t end) {
    ...
    if (end >= _recordNum) end = _recordNum - 1;
    bFind = true;
  ...
  int32_t LeafPage::SearchKey(const RawKey& key, bool& bFind, bool bInc, int32_t start, int32_t end) {
    if (_recordNum == 0) {
      bFind = false;
      return 0;
    }
    ...
    while (true) {
      ...
      if (hr < 0) {
        ...
        if (bUnique) {
          ...
          if (bInc) {
            if (middle > start && (_vctRecord.size() > 0 ?
            ...
            if (middle < end && (_vctRecord.size() > 0 ?
              GetVctRecord(middle + 1)->CompareKey(key) == 0 : CompareTo(middle + 1, key) == 0)) {
              start = middle + 1;
            }
            else {
