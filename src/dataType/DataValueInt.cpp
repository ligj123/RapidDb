...
DataValueInt::DataValueInt(Byte *byArray, bool bKey)
    : IDataValue(DataType::INT, ValueType::BYTES_VALUE, bKey),
      byArray_(byArray) {}
...
DataValueInt::DataValueInt(std::any val, bool bKey)
  ...
  if (val.type() == typeid(int64_t))
    soleValue_ = (int32_t)std::any_cast<int64_t>(val);
    ...
    soleValue_ = (int32_t)std::any_cast<uint64_t>(val);
    ...
    throw utils::ErrorMsg(DT_UNSUPPORT_CONVERT,
                          {val.type().name(), "DataValueInt"});
}

DataValueInt::DataValueInt(const DataValueInt &src) : IDataValue(src) {
  switch (src.valType_) {
...
uint32_t DataValueInt::WriteData(Byte *buf) { return WriteData(buf, bKey_); }

uint32_t DataValueInt::GetPersistenceLength(bool key) const {
  return key ? sizeof(int32_t)
             : (valType_ == ValueType::NULL_VALUE ? 1 : 1 + sizeof(int32_t));
}
...
uint32_t DataValueInt::WriteData(fstream &fs) {
  if (valType_ == ValueType::NULL_VALUE) {
    fs.put((Byte)DataType::INT & DATE_TYPE);
    return 1;
  } else {
    fs.put(VALUE_TYPE | ((Byte)DataType::INT & DATE_TYPE));
    if (valType_ == ValueType::SOLE_VALUE) {
      fs.write((char *)&soleValue_, sizeof(int32_t));
    } else {
      fs.write((char *)byArray_, sizeof(int32_t));
    }
    return sizeof(int32_t) + 1;
  }
}

uint32_t DataValueInt::ReadData(fstream &fs) {
  Byte by;
  fs.read((char *)&by, 1);
  valType_ =
  ...
  fs.read((char *)&soleValue_, sizeof(int32_t));
  return sizeof(int32_t) + 1;
}

uint32_t DataValueInt::ReadData(Byte *buf, uint32_t len, bool bSole) {
  if (bKey_) {
...
uint32_t DataValueInt::GetDataLength() const {
  return bKey_ ? sizeof(int32_t)
               : (valType_ == ValueType::NULL_VALUE ? 0 : sizeof(int32_t));
}

uint32_t DataValueInt::GetMaxLength() const { return sizeof(int32_t); }
...
bool DataValueInt::operator>(const DataValueInt &dv) const {
  ...
  int32_t v1 = (valType_ == ValueType::SOLE_VALUE
                    ? soleValue_
                    : utils::Int32FromBytes(byArray_, bKey_));
  int32_t v2 = (dv.valType_ == ValueType::SOLE_VALUE
                    ? dv.soleValue_
                    : utils::Int32FromBytes(dv.byArray_, dv.bKey_));
  return v1 > v2;
}
...
void DataValueInt::ToString(StrBuff &sb) {
  if (valType_ == ValueType::NULL_VALUE) {
    return;
  }
  if (12 > sb.GetFreeLen()) {
  ...
  int32_t val = (valType_ == ValueType::SOLE_VALUE
                     ? soleValue_
                     : utils::Int32FromBytes(byArray_, bKey_));
