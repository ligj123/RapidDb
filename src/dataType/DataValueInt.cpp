...
namespace storage {
	...
	DataValueInt::DataValueInt(char* byArray, bool bKey)
		...
		: IDataValue(DataType::INT, ValueType::SOLE_VALUE, bKey)
	{
		...
		else throw utils::ErrorMsg(2001, { val.type().name(), "DataValueInt" });
	}
	...
	uint32_t DataValueInt::WriteData(char* buf)
	{
		if (bKey_)
		{
			if (valType_ == ValueType::NULL_VALUE)
			{
				std::memset(buf, 0, sizeof(int32_t));
			}
	...
	uint32_t DataValueInt::ReadData(char* buf, uint32_t len)
	{
		if (bKey_)
		{
			if (len == -1)
	...
	uint32_t DataValueInt::GetLength() const
	{
	...
	{
		return sizeof(int32_t) + (bKey_ ? 0 : 1);
	}
