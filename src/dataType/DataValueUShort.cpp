...
DataValueUShort::DataValueUShort(Byte *byArray, bool bKey)
    : IDataValue(DataType::USHORT, ValueType::BYTES_VALUE, bKey),
      byArray_(byArray) {}

DataValueUShort::DataValueUShort(std::any val, bool bKey)
    : IDataValue(DataType::USHORT, ValueType::SOLE_VALUE, bKey) {
  if (val.type() == typeid(int64_t))
    soleValue_ = (uint16_t)std::any_cast<int64_t>(val);
  else if (val.type() == typeid(int32_t))
    soleValue_ = (uint16_t)std::any_cast<int32_t>(val);
  else if (val.type() == typeid(int16_t))
    soleValue_ = std::any_cast<int16_t>(val);
  else if (val.type() == typeid(uint64_t))
    soleValue_ = (uint16_t)std::any_cast<uint64_t>(val);
  else if (val.type() == typeid(uint32_t))
    soleValue_ = (uint16_t)std::any_cast<uint32_t>(val);
  else if (val.type() == typeid(uint16_t))
    soleValue_ = std::any_cast<uint16_t>(val);
    ...
    soleValue_ = (uint16_t)std::stoi(std::any_cast<std::string>(val));
  else
    throw utils::ErrorMsg(DT_UNSUPPORT_CONVERT,
                          {val.type().name(), "DataValueUShort"});
}

DataValueUShort::DataValueUShort(const DataValueUShort &src) : IDataValue(src) {
  switch (src.valType_) {
...
DataValueUShort *DataValueUShort::CloneDataValue(bool incVal) {
  return new DataValueUShort(*this);
}
...
uint32_t DataValueUShort::WriteData(Byte *buf, bool key) {
  if (key) {
    ...
    if (valType_ == ValueType::NULL_VALUE) {
      *buf = (Byte)DataType::USHORT & DATE_TYPE;
      return 1;
    } else if (valType_ == ValueType::BYTES_VALUE) {
      *buf = VALUE_TYPE | ((Byte)DataType::USHORT & DATE_TYPE);
      buf++;
      std::memcpy(buf, byArray_, sizeof(uint16_t));
...
uint32_t DataValueUShort::WriteData(fstream &fs) {
  if (valType_ == ValueType::NULL_VALUE) {
    fs.put((Byte)DataType::USHORT & DATE_TYPE);
    return 1;
  } else {
    fs.put(VALUE_TYPE | ((Byte)DataType::USHORT & DATE_TYPE));
    if (valType_ == ValueType::SOLE_VALUE) {
      fs.write((char *)&soleValue_, sizeof(uint16_t));
    } else {
      fs.write((char *)byArray_, sizeof(uint16_t));
    }
    return sizeof(uint16_t) + 1;
  }
}

uint32_t DataValueUShort::ReadData(fstream &fs) {
  Byte by;
  fs.read((char *)&by, 1);
  valType_ =
  ...
  fs.read((char *)&soleValue_, sizeof(uint16_t));
  return sizeof(uint16_t) + 1;
}

uint32_t DataValueUShort::ReadData(Byte *buf, uint32_t len, bool bSole) {
  if (bKey_) {
    valType_ = ValueType::SOLE_VALUE;
    soleValue_ = utils::UInt16FromBytes(buf, bKey_);
    return sizeof(uint16_t);
  } else {
    valType_ =
...
uint32_t DataValueUShort::GetDataLength() const {
  return bKey_ ? sizeof(uint16_t)
               : (valType_ == ValueType::NULL_VALUE ? 0 : sizeof(uint16_t));
}

uint32_t DataValueUShort::GetMaxLength() const { return sizeof(uint16_t); }
...
bool DataValueUShort::operator>(const DataValueUShort &dv) const {
  ...
  uint16_t v1 = (valType_ == ValueType::SOLE_VALUE
                     ? soleValue_
                     : utils::UInt16FromBytes(byArray_, bKey_));
  uint16_t v2 = (dv.valType_ == ValueType::SOLE_VALUE
                     ? dv.soleValue_
                     : utils::UInt16FromBytes(dv.byArray_, dv.bKey_));
  return v1 > v2;
}
...
void DataValueUShort::ToString(StrBuff &sb) {
  ...
  if (9 > sb.GetFreeLen()) {
    sb.Resize(sb.GetStrLen() + 9);
  }

  uint16_t val = (valType_ == ValueType::SOLE_VALUE
                      ? soleValue_
                      : utils::UInt16FromBytes(byArray_, bKey_));
  char *dest = sb.GetFreeBuff();
  int n = sprintf(dest, "%u", val);
