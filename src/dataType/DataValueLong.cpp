...
DataValueLong::DataValueLong(Byte *byArray, bool bKey)
    : IDataValue(DataType::LONG, ValueType::BYTES_VALUE, bKey),
      byArray_(byArray) {}
...
DataValueLong::DataValueLong(std::any val, bool bKey)
  ...
  if (val.type() == typeid(int64_t))
    soleValue_ = std::any_cast<int64_t>(val);
    ...
    soleValue_ = std::any_cast<uint64_t>(val);
    ...
    soleValue_ = std::stoll(std::any_cast<std::string>(val));
  else
    throw utils::ErrorMsg(DT_UNSUPPORT_CONVERT,
                          {val.type().name(), "DataValueLong"});
}

DataValueLong::DataValueLong(const DataValueLong &src) : IDataValue(src) {
  switch (src.valType_) {
...
uint32_t DataValueLong::WriteData(fstream &fs) {
  if (valType_ == ValueType::NULL_VALUE) {
    fs.put((Byte)DataType::LONG & DATE_TYPE);
    return 1;
  } else {
    fs.put(VALUE_TYPE | ((Byte)DataType::LONG & DATE_TYPE));
    if (valType_ == ValueType::SOLE_VALUE) {
      fs.write((char *)&soleValue_, sizeof(int64_t));
    } else {
      fs.write((char *)byArray_, sizeof(int64_t));
    }
    return sizeof(int64_t) + 1;
  }
}

uint32_t DataValueLong::ReadData(fstream &fs) {
  Byte by;
  fs.read((char *)&by, 1);
  valType_ =
  ...
  fs.read((char *)&soleValue_, sizeof(int64_t));
  return sizeof(int64_t) + 1;
}

uint32_t DataValueLong::ReadData(Byte *buf, uint32_t len, bool bSole) {
  if (bKey_) {
...
uint32_t DataValueLong::GetDataLength() const {
  return bKey_ ? sizeof(int64_t)
               : (valType_ == ValueType::NULL_VALUE ? 0 : sizeof(int64_t));
}

uint32_t DataValueLong::GetMaxLength() const { return sizeof(int64_t); }
...
bool DataValueLong::operator>(const DataValueLong &dv) const {
  ...
  int64_t v1 = (valType_ == ValueType::SOLE_VALUE
                    ? soleValue_
                    : utils::Int64FromBytes(byArray_, bKey_));
  int64_t v2 = (dv.valType_ == ValueType::SOLE_VALUE
                    ? dv.soleValue_
                    : utils::Int64FromBytes(dv.byArray_, dv.bKey_));
  return v1 > v2;
}
...
void DataValueLong::ToString(StrBuff &sb) const {
  ...
  int64_t val = (valType_ == ValueType::SOLE_VALUE
                     ? soleValue_
                     : utils::Int64FromBytes(byArray_, bKey_));
  char *dest = sb.GetFreeBuff();
  int n = sprintf(dest, "%lld", val);
