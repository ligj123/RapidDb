...
DataValueDouble::DataValueDouble(Byte *byArray, bool bKey)
    : IDataValue(DataType::DOUBLE, ValueType::BYTES_VALUE, bKey),
      byArray_(byArray) {}

DataValueDouble::DataValueDouble(std::any val, bool bKey)
    : IDataValue(DataType::DOUBLE, ValueType::SOLE_VALUE, bKey) {
  if (val.type() == typeid(double))
    soleValue_ = std::any_cast<double>(val);
  else if (val.type() == typeid(float))
    soleValue_ = std::any_cast<float>(val);
  else if (val.type() == typeid(int64_t))
    soleValue_ = (double)std::any_cast<int64_t>(val);
    ...
    soleValue_ = (double)std::any_cast<uint64_t>(val);
    ...
    soleValue_ = std::stod(std::any_cast<std::string>(val));
  else
    throw utils::ErrorMsg(DT_UNSUPPORT_CONVERT,
                          {val.type().name(), "DataValueDouble"});
}

DataValueDouble::DataValueDouble(const DataValueDouble &src) : IDataValue(src) {
  switch (src.valType_) {
...
uint32_t DataValueDouble::WriteData(Byte *buf, bool key) {
  if (key) {
    ...
    if (valType_ == ValueType::NULL_VALUE) {
      *buf = (Byte)DataType::DOUBLE & DATE_TYPE;
      return 1;
    } else if (valType_ == ValueType::BYTES_VALUE) {
      *buf = VALUE_TYPE | ((Byte)DataType::DOUBLE & DATE_TYPE);
      buf++;
      std::memcpy(buf, byArray_, sizeof(double));
...
uint32_t DataValueDouble::WriteData(fstream &fs) {
  if (valType_ == ValueType::NULL_VALUE) {
    fs.put((Byte)DataType::DOUBLE & DATE_TYPE);
    return 1;
  } else {
    fs.put(VALUE_TYPE | ((Byte)DataType::DOUBLE & DATE_TYPE));
    if (valType_ == ValueType::SOLE_VALUE) {
      fs.write((char *)&soleValue_, sizeof(double));
    } else {
      fs.write((char *)byArray_, sizeof(double));
    }
    return sizeof(double) + 1;
  }
}

uint32_t DataValueDouble::ReadData(fstream &fs) {
  Byte by;
  fs.read((char *)&by, 1);
  valType_ =
  ...
  fs.read((char *)&soleValue_, sizeof(double));
  return sizeof(double) + 1;
}

uint32_t DataValueDouble::ReadData(Byte *buf, uint32_t len, bool bSole) {
  if (bKey_) {
...
uint32_t DataValueDouble::GetDataLength() const {
  return bKey_ ? sizeof(double)
               : (valType_ == ValueType::NULL_VALUE ? 0 : sizeof(double));
}

uint32_t DataValueDouble::GetMaxLength() const { return sizeof(double); }
...
bool DataValueDouble::operator>(const DataValueDouble &dv) const {
  ...
  double v1 = (valType_ == ValueType::SOLE_VALUE
                   ? soleValue_
                   : utils::DoubleFromBytes(byArray_, bKey_));
  double v2 = (dv.valType_ == ValueType::SOLE_VALUE
                   ? dv.soleValue_
                   : utils::DoubleFromBytes(dv.byArray_, dv.bKey_));
  return v1 > v2;
}
...
void DataValueDouble::ToString(StrBuff &sb) {
  ...
  if (25 > sb.GetFreeLen()) {
    sb.Resize(sb.GetStrLen() + 25);
  }

  double d = (valType_ == ValueType::SOLE_VALUE
                  ? soleValue_
                  : utils::DoubleFromBytes(byArray_, bKey_));
  char *dest = sb.GetFreeBuff();
  int n = sprintf(dest, "%f", d);
