...
DataValueFixChar::DataValueFixChar(char *val, uint32_t maxLength, bool bKey)
    : IDataValue(DataType::FIXCHAR, ValueType::SOLE_VALUE, bKey),
      soleValue_(val), soleLength_((uint32_t)strlen(val) + 1),
      maxLength_(maxLength) {
...
DataValueFixChar::DataValueFixChar(const char *val, uint32_t maxLength,
                                   bool bKey)
    : IDataValue(DataType::FIXCHAR, ValueType::SOLE_VALUE, bKey),
...
DataValueFixChar::DataValueFixChar(Byte *byArray, uint32_t strLen,
                                   uint32_t maxLength, bool bKey)
    : IDataValue(DataType::FIXCHAR, ValueType::BYTES_VALUE, bKey),
      byArray_(byArray), soleLength_(strLen), maxLength_(maxLength) {}
...
uint32_t DataValueFixChar::WriteData(Byte *buf, bool key) {
  if (key) {
    ...
    if (valType_ == ValueType::BYTES_VALUE) {
      ...
      std::memset(buf + strlen(soleValue_), '\0',
                  maxLength_ - strlen(soleValue_));
    }
