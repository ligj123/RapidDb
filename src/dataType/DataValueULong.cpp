...
DataValueULong::DataValueULong(Byte *byArray, bool bKey)
    : IDataValue(DataType::ULONG, ValueType::BYTES_VALUE, bKey),
      byArray_(byArray) {}
...
DataValueULong::DataValueULong(std::any val, bool bKey)
  ...
  if (val.type() == typeid(int64_t))
    ...
    throw utils::ErrorMsg(DT_UNSUPPORT_CONVERT,
                          {val.type().name(), "DataValueULong"});
}

DataValueULong::DataValueULong(const DataValueULong &src) : IDataValue(src) {
  switch (src.valType_) {
...
uint32_t DataValueULong::WriteData(Byte *buf) { return WriteData(buf, bKey_); }
...
uint32_t DataValueULong::WriteData(Byte *buf, bool key) {
  if (key) {
    ...
    if (valType_ == ValueType::NULL_VALUE) {
      *buf = (Byte)DataType::ULONG & DATE_TYPE;
      return 1;
    } else if (valType_ == ValueType::BYTES_VALUE) {
      *buf = VALUE_TYPE | ((Byte)DataType::ULONG & DATE_TYPE);
      buf++;
      std::memcpy(buf, byArray_, sizeof(uint64_t));
...
uint32_t DataValueULong::GetMaxLength() const { return sizeof(uint64_t); }
...
bool DataValueULong::operator>(const DataValueULong &dv) const {
  ...
  uint64_t v2 = (dv.valType_ == ValueType::SOLE_VALUE
                     ? dv.soleValue_
                     : utils::UInt64FromBytes(dv.byArray_, dv.bKey_));
  return v1 > v2;
}
