...
DataValueShort::DataValueShort(Byte *byArray, bool bKey)
    : IDataValue(DataType::SHORT, ValueType::BYTES_VALUE, bKey),
      byArray_(byArray) {}
...
DataValueShort::DataValueShort(std::any val, bool bKey)
  ...
  if (val.type() == typeid(int64_t))
    ...
    throw utils::ErrorMsg(DT_UNSUPPORT_CONVERT,
                          {val.type().name(), "DataValueShort"});
}

DataValueShort::DataValueShort(const DataValueShort &src) : IDataValue(src) {
  switch (src.valType_) {
...
uint32_t DataValueShort::WriteData(Byte *buf) { return WriteData(buf, bKey_); }

uint32_t DataValueShort::GetPersistenceLength(bool key) const {
  return key ? sizeof(int16_t)
             : (valType_ == ValueType::NULL_VALUE ? 1 : 1 + sizeof(int16_t));
}
...
uint32_t DataValueShort::WriteData(Byte *buf, bool key) {
  if (key) {
    ...
    if (valType_ == ValueType::NULL_VALUE) {
      *buf = (Byte)DataType::SHORT & DATE_TYPE;
      return 1;
    } else if (valType_ == ValueType::BYTES_VALUE) {
      *buf = VALUE_TYPE | ((Byte)DataType::SHORT & DATE_TYPE);
      buf++;
      std::memcpy(buf, byArray_, sizeof(int16_t));
...
uint32_t DataValueShort::WriteData(fstream &fs) {
  if (valType_ == ValueType::NULL_VALUE) {
    fs.put((Byte)DataType::SHORT & DATE_TYPE);
    return 1;
  } else {
    fs.put(VALUE_TYPE | ((Byte)DataType::SHORT & DATE_TYPE));
    if (valType_ == ValueType::SOLE_VALUE) {
      fs.write((char *)&soleValue_, sizeof(int16_t));
    } else {
      fs.write((char *)byArray_, sizeof(int16_t));
    }
    return sizeof(int16_t) + 1;
  }
}

uint32_t DataValueShort::ReadData(fstream &fs) {
  Byte by;
  fs.read((char *)&by, 1);
  valType_ =
  ...
  fs.read((char *)&soleValue_, sizeof(int16_t));
  return sizeof(int16_t) + 1;
}

uint32_t DataValueShort::ReadData(Byte *buf, uint32_t len, bool bSole) {
  if (bKey_) {
...
uint32_t DataValueShort::GetDataLength() const {
  return bKey_ ? sizeof(int16_t)
               : (valType_ == ValueType::NULL_VALUE ? 0 : sizeof(int16_t));
}

uint32_t DataValueShort::GetMaxLength() const { return sizeof(int16_t); }
...
bool DataValueShort::operator>(const DataValueShort &dv) const {
  ...
  int16_t v1 = (valType_ == ValueType::SOLE_VALUE
                    ? soleValue_
                    : utils::Int16FromBytes(byArray_, bKey_));
  int16_t v2 = (dv.valType_ == ValueType::SOLE_VALUE
                    ? dv.soleValue_
                    : utils::Int16FromBytes(dv.byArray_, dv.bKey_));
  return v1 > v2;
}
...
void DataValueShort::ToString(StrBuff &sb) {
  if (valType_ == ValueType::NULL_VALUE) {
    return;
  }
  if (9 > sb.GetFreeLen()) {
  ...
  int16_t val = (valType_ == ValueType::SOLE_VALUE
                     ? soleValue_
                     : utils::Int16FromBytes(byArray_, bKey_));
  char *dest = sb.GetFreeBuff();
  int n = sprintf(dest, "%d", val);
