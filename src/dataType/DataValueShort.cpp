...
namespace storage {
	...
	DataValueShort::DataValueShort(char* byArray, bool bKey)
		...
		: IDataValue(DataType::SHORT, ValueType::SOLE_VALUE, bKey)
	{
		...
		else throw utils::ErrorMsg(2001, { val.type().name(), "DataValueShort" });
	}
	...
	uint32_t DataValueShort::WriteData(char* buf)
	{
		if (bKey_)
		{
			if (valType_ == ValueType::NULL_VALUE)
			{
				std::memset(buf, 0, sizeof(int16_t));
			}
	...
	uint32_t DataValueShort::ReadData(char* buf, uint32_t len)
	{
		if (bKey_)
		{
			if (len == -1)
	...
	uint32_t DataValueShort::GetLength() const
	{
	...
	{
		return sizeof(int16_t) + (bKey_ ? 0 : 1);
	}
