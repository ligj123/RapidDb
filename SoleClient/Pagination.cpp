...
void Pagination::display() {
  ...
  keypad(stdscr, TRUE);
  curs_set(0);
  // The time (in ms) that getch() waits for input. Having a timeout is
  // important for catching a forwarded CTRL-C.
  timeout(1000);
...
void Pagination::_print_page(size_t first_line, size_t first_column) {
  ...
  printw("Press 'q' to quit. ARROW KEYS, PAGE UP/DOWN, for navigation. 'h' for "
