...
{
  const string FILE_NAME = "./dbTest/testBranchRecord" + utils::StrMSTime() + ".dat";
  ...
  Byte buff[100];
  uint16_t pos = 0;
  uint16_t len = (uint16_t)(sizeof(uint16_t) * 2 + dvKey->GetPersistenceLength(true) + BranchRecord::PAGE_ID_LEN);
...
{
  const string FILE_NAME = "./dbTest/testBranchRecordEqual" + utils::StrMSTime() + ".dat";
  const string TABLE_NAME = "testTable";
  ...
  BOOST_TEST(rr1->CompareTo(rr3) == 0);
