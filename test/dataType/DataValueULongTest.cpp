...
#include "../../src/dataType/DataValueULong.h"

namespace storage {
	BOOST_AUTO_TEST_SUITE(DataTypeTest)

	BOOST_AUTO_TEST_CASE(DataValueULong_test)
	{
		DataValueULong dv1;
	
		BOOST_TEST(dv1.GetDataType() == DataType::ULONG);
		...
		DataValueULong dv2(true);
		BOOST_TEST(dv2.GetMaxLength() == 8);
		BOOST_TEST(dv2.GetLength() == 8);
		BOOST_TEST(dv2.GetPersistenceLength() == 8);
		BOOST_TEST(dv1 == dv2);

		DataValueULong dv3(2, false);
		BOOST_TEST(dv3.GetDataType() == DataType::ULONG);
		BOOST_TEST(dv3.GetValueType() == ValueType::SOLE_VALUE);
		BOOST_TEST(!dv3.IsNull());
		BOOST_TEST(dv3.GetLength() == 8);
		...
		DataValueULong dv4(2, true);
		BOOST_TEST(dv4.GetPersistenceLength() == 8);
		BOOST_TEST(dv4 == dv3);

		char buf[100];
		*((uint64_t*)buf) = 10;
		DataValueULong dv5(buf, false);
		...
		DataValueULong dv8(dv7);
		BOOST_TEST(dv8 == dv7);

		dv6 = dv7;
		BOOST_TEST(dv6 == dv7);

		DataValueULong dv9(std::any(100));
		BOOST_TEST((uint64_t)dv9 == 100);
	}
