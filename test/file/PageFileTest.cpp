...
namespace storage {
	...
	BOOST_AUTO_TEST_CASE(PageFile_test)
	{
		...
		for (uint32_t i = 0; i < pageLen; i++) bys1[i] = (char)i;
		...
		for (uint32_t i = 0; i < pageLen; i++) bys1[i] = (char)(i + 10);
		pf.WritePage(pageLen * 2, bys1, pageLen);
	...
	BOOST_AUTO_TEST_CASE(OverflowFile_test)
	{
		...
		vctDv.push_back(DataValueFactory(DataType::FIXCHAR, false, 100, "abcdefghigklmn"));
		vctDv.push_back(DataValueFactory(DataType::VARCHAR, false, 100, "1234567890abcdefghijklmn"));
		uint32_t len = 0;
