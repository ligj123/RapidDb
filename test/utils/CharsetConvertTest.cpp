...
namespace utils {
	...
	BOOST_AUTO_TEST_CASE(CharsetConvert_test)
	{
		...
		ConvResult rt = CodeConvert::AsciiToUTF8(pAsc, (int)std::strlen(pAsc), tmp, len, Charsets::GBK, true);
		BOOST_TEST(rt == ConvResult::OK);
		BOOST_TEST(strcmp(tmp, pUtf8) == 0);
		BOOST_TEST(tmp[strlen(tmp) + 1] == 0);

		len = 10;
		rt = CodeConvert::AsciiToUTF8(pAsc, (int)std::strlen(pAsc), tmp, len, Charsets::GBK, false);
		BOOST_TEST(rt == ConvResult::PARTIAL);
		BOOST_TEST(len == 10);
		BOOST_TEST(tmp[len] != '\0');

		len = 100;
		rt = CodeConvert::AsciiFromUTF8(pUtf8, (int)std::strlen(pUtf8), tmp, len, Charsets::GBK, true);
		BOOST_TEST(rt == ConvResult::OK);
		BOOST_TEST(strcmp(tmp, pAsc) == 0);
		BOOST_TEST(tmp[strlen(tmp) + 1] == 0);

		len = 10;
		rt = CodeConvert::AsciiFromUTF8(pUtf8, (int)std::strlen(pUtf8), tmp, len, Charsets::GBK, false);
		BOOST_TEST(rt == ConvResult::PARTIAL);
		BOOST_TEST(len == 10);
		BOOST_TEST(tmp[len] != '\0');
		...
		len = 100;
		rt = CodeConvert::MultiCharFromWideChar(pUncode, (int)wcslen(pUncode), tmp, len, Charsets::GBK, true);
		BOOST_TEST(rt == ConvResult::OK);
		BOOST_TEST(strcmp(tmp, pAsc) == 0);
		BOOST_TEST(tmp[len] == 0);
	}
