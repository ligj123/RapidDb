...
PhysTable *CreateTestTable(Database *db, const MString &tableName, int rowNum) {
  ...
  DataValueFixChar dvFix1000(fix1000.c_str(), 999, 1000);
  dvFix1000.SetConstRef();

  MString var1000 = "VARCHAR_1000_" + MString(987, 'a');
  DataValueVarChar dvVar1000(var1000.c_str(), 999, 1000);
  dvVar1000.SetConstRef();

  MString fix50 = "FIXCHAR_50_" + MString(37, 'a');
  DataValueFixChar dvFix50(fix50.c_str(), 49, 50);
  dvFix50.SetConstRef();
  ...
  for (int i = 0; i < rowNum; i++) {
    ...
    IndexPage *idxPage = priTree->GetRootPage();
    ...
    lp->AddWriteQueue(priMap);
    if (lp->NeedForceSplit()) {
      lp->SplitPage(priMap, UINT8_MAX);
    }
    ...
    idxPage = nonTree->GetRootPage();
    b = nonTree->SearchPage(*nlr, idxPage);
    assert(b && idxPage->GetPageType() == PageType::LEAF_PAGE);
    lp = (LeafPage *)idxPage;
    pos = lp->SearchRecord(*nlr, bFind);
    lp->InsertRecord(nlr, pos);
    lp->AddWriteQueue(nonMap);
    if (lp->NeedForceSplit()) {
      lp->SplitPage(nonMap, UINT8_MAX);
    }
  }

  IndexTree::SettleUpdatedPages(priMap);
  IndexTree::SettleUpdatedPages(uniMap);
  IndexTree::SettleUpdatedPages(nonMap);
